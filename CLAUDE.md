...
**Pré-cadastro rápido de paciente (`src/app/patient/patient-quick-create/`):** when scheduling a new appointment for a patient not yet registered, `appointment-edit.component.ts`'s `openQuickCreateDialog()` (called from the 404 catch branch of `searchPatientByCpf()`/`searchPatientById()`) opens `PatientQuickCreateComponent`, a minimal dialog collecting only `fullName` + `principalNumber` (plus `guardianName`, required only if the "menor de idade" checkbox is on) via `PatientService.createQuick()` (`POST /patient/quick`). This produces a patient with backend type `NOT_IDENTIFIED` — reusing the domain's existing incomplete-patient concept, not a new one — so it shows up highlighted in the Pacientes list like any other unidentified patient. Later, completing that patient's registration (`patient-edit.component.ts`'s `save()`, the `update()` branch) can hit a `409` if the CPF entered already belongs to a different patient; `handleMergeConflict()` then fetches the conflicting patient via `getByDocument('CPF', ...)` and shows `ConfirmDialogComponent` before calling `PatientService.merge(sourceId, targetId)` (`POST /patient/{sourceId}/merge/{targetId}`) — never merge without that confirmation step, since it deletes the duplicate and reassigns its attendance/appointment/allergy history.
